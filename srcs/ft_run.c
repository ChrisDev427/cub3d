...
/*   Updated: 2023/06/06 15:53:48 by axfernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	game->image.ray_img.img = mlx_new_image(game->win.mlx, WIDTH, HEIGHT);
	game->ray.x = 0;
	double ratio = 0;
	//printf("win x = %d\n", game->win.x);
	...
	while (game->ray.x < 1920)
	{
		...
		{
			ft_my_mlx_pixel_put(&game->image.ray_img, game->ray.x, game->ray.lineheight, 0xFFFFFF);
			//mlx_pixel_put(game->win.mlx, game->win.win, game->ray.drawstart, game->ray.drawend, 0xFFFFFF);
			game->ray.drawstart++;
		}
		game->ray.x++;
	}
    	mlx_put_image_to_window(game->win.mlx, game->win.win, game->image.ray_img.img, game->ray.drawstart, game->ray.drawend);
}
